...
def get_args():
    ...
    parser.add_argument("--system_prompt_column_name", type=str, default="system_prompt", help="")
...
def process_batch(batch, llm, params, tokenizer=None, args=None):
    ...
    user_inputs = [
        {
            "system_prompt": item.get(args.system_prompt_column_name, None),
            "instruction": item[args.instruct_column_name],
        }
        for item in batch
    ]
    ...
    if args.prompt_file is not None:
        ...
        for user_input in user_inputs:
            user_input["instruction"] = prompt_template.format(text=user_input["instruction"])
    ...
    for user_input in user_inputs:
        ...
        if user_input["system_prompt"] is not None:
    ...
    for i, item in enumerate(batch):
        if args.engine == "vllm":
            ...
            item[args.output_column_name] = output
        elif args.engine == "hf":
