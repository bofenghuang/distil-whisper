...
def main():
    ...
    if data_args.id_column_name not in list(next(iter(raw_datasets.values())).features.keys()):
        with accelerator.local_main_process_first():
            ...
            #     lambda _, idx: {data_args.id_column_name: f"{idx:09d}"},
            #     with_indices=True,
    ...
    def eval_step_with_save(split="eval"):
        ...
        # wer_ortho = 100 * metric.compute(predictions=raw_datasets[split]["whisper_transcript"], references=raw_datasets[split][text_column_name])
